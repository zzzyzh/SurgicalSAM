...
def _build_lora_sam_hier(
    model_type=None,
    use_image_encoder=True,
    use_mask_decoder=True,
    ...
    if checkpoint is not None:
        ...
        if use_image_encoder:
            ...
            encoder_state_dict = {k.split("image_encoder.")[-1]: v for k, v in new_state_dict.items() if k.startswith("image_encoder")}
            image_encoder.load_state_dict(encoder_state_dict)
         
        # only filter the weight of the mask_decoder
        if use_mask_decoder:
            decoder_state_dict = {k.split("mask_decoder.")[-1]: v for k, v in new_state_dict.items() if k.startswith("mask_decoder.")}
